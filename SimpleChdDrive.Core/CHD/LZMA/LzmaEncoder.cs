using SimpleChdDrive.Core.CHD.LZMA.LZ;
...
internal class Encoder
{
    ...
    private static uint GetPosSlot(uint pos)
    {
        ...
        {
            < 1 << 11 => GFastPos[pos],
            < 1 << 21 => (uint)(GFastPos[pos >> 10] + 20),
            _ => (uint)(GFastPos[pos >> 20] + 40)
        };
    }
    ...
    private static uint GetPosSlot2(uint pos)
    {
        return pos switch
        {
            < 1 << 17 => (uint)(GFastPos[pos >> 6] + 12),
            < 1 << 27 => (uint)(GFastPos[pos >> 16] + 32),
            _ => (uint)(GFastPos[pos >> 26] + 52)
        };
    }
    ...
    private MatchFinderType _matchFinderType = MatchFinderType.Bt4;
    ...
    private uint GetRepLen1Price(LzmaState state, uint posState)
    {
        return _isRepG0[state.Index].GetPrice0() +
               _isRep0Long[(state.Index << Base.KNumPosStatesBitsMax) + posState].GetPrice0();
    }

    private uint GetPureRepPrice(uint repIndex, LzmaState state, uint posState)
    {
        uint price;
        if (repIndex == 0)
        {
    ...
    private uint GetRepPrice(uint repIndex, uint len, LzmaState state, uint posState)
    {
...
#pragma warning restore CA1505
    ...
    public long Code(Stream inStream, bool final)
    {
        ...
        try
        {
            while (true)
            {
                CodeOneBlock(out var processedInSize, out _, out var finished);
                if (finished)
        ...
        for (uint i = 0; i < properties.Length; i++)
        {
            ...
            switch (propIDs[i])
            {
                ...
                case CoderPropId.MatchFinder:
                {
                    ...
                    _matchFinderType = (MatchFinderType)m;
