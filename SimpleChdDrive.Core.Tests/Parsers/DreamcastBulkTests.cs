...
public class DreamcastBulkTests
{
    ...
    public DreamcastBulkTests(ITestOutputHelper output)
    {
        _output = output;
    }

    [Fact]
    public void ParseEntireDreamcastLibrary()
    {
        if (!Directory.Exists(LibraryPath))
        {
            _output.WriteLine($"SKIP: {LibraryPath} not found");
            return;
        }

        var chdFiles = Directory.EnumerateFiles(LibraryPath, "*.chd").OrderBy(static f => f).ToList();
        _output.WriteLine($"Found {chdFiles.Count} CHD files");
