...
public class SaturnIntegrationTests
{
    ...
    public SaturnIntegrationTests(ITestOutputHelper output)
    {
        _output = output;
    }
    ...
    {
        return SequentialTestRunner.CollectPaths(ChdPathCatalog.SegaSaturn.Paths);
    }

    [Fact]
    public void Iso9660ParserParsesSaturnDisc()
    {
        var paths = GetPaths();
        SequentialTestRunner.Run(_output, nameof(Iso9660ParserParsesSaturnDisc), paths, static (path, output) =>
        {
    ...
    [Fact]
    public void SegaSaturnParserParsesSaturnDisc()
    {
        var paths = GetPaths();
        SequentialTestRunner.Run(_output, nameof(SegaSaturnParserParsesSaturnDisc), paths, static (path, output) =>
        {
            var err = ChdFile.Open(path, out var chd);
            Assert.Equal(ChdError.Chderrnone, err);
            Assert.NotNull(chd);
            ...
            try
            {
                ...
                var root = new FsNode();
                var parser = new SegaSaturnParser(reader);

                var ok = parser.Parse(root);
                output.WriteLine($"SegaSaturnParser: {(ok ? "OK" : "FAILED")}");

                Assert.True(ok, "SegaSaturnParser could not parse the disc");
    ...
    [Fact]
    public void ChdContainerMountAndParseSaturnDisc()
    {
        var paths = GetPaths();
        SequentialTestRunner.Run(_output, nameof(ChdContainerMountAndParseSaturnDisc), paths, static (path, output) =>
        {
            var container = new ChdContainer(path);
            try
            {
                Assert.True(container.MountAndParse(ConsoleType.Saturn), "MountAndParse failed");
