...
public class NeoGeoCdIntegrationTests
{
    private readonly ITestOutputHelper _output;

    public NeoGeoCdIntegrationTests(ITestOutputHelper output)
    {
        _output = output;
    }
    ...
    {
        return SequentialTestRunner.CollectPaths(ChdPathCatalog.NeoGeoCd.Paths);
    }
    ...
    public void NeoGeoCdParserParsesNeoGeoCdDisc()
    {
        ...
        SequentialTestRunner.Run(_output, nameof(NeoGeoCdParserParsesNeoGeoCdDisc), paths, static (path, output) =>
        {
            ...
            try
            {
                ...
                var root = new FsNode();
                var parser = new NeoGeoCdParser(reader);

                var ok = parser.Parse(root);
                output.WriteLine($"NeoGeoCdParser: {(ok ? "OK" : "FAILED")}");

                Assert.True(ok, "NeoGeoCdParser could not parse the disc");
    ...
    public void ChdContainerMountAndParseNeoGeoCdDisc()
    {
        ...
        SequentialTestRunner.Run(_output, nameof(ChdContainerMountAndParseNeoGeoCdDisc), paths, static (path, output) =>
        {
            ...
            try
            {
                Assert.True(container.MountAndParse(ConsoleType.NeoGeoCd), "MountAndParse failed");
